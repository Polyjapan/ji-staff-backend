...
object TemporaryEdition {
  val pages: List[FormPage] = {
    ...
    val activities = List("Oui je suis motivé pour organiser des activités", "Oui je peux aider à des activités mais pas les organiser", "Non je préfère aider ailleurs")
    ...
    val motivations = FormPage(4, minorOnly = false, "Motivation & Remarques", List(
      ...
      FormField("activities", "Serais-tu motivé pour animer une salle d'activités ?", "Pour organiser des jeux, karaokés, etc", order = 2, controlType = "option", additionalData = Json.obj("values" -> activities)).withValidator(SetContainedValidator("Organisation des activités invalide", activities.toSet)),
      FormField("remarks", "Remarques", "Un truc de plus à nous dire ?", order = 3, required = false, controlType = "textarea")
    ))
